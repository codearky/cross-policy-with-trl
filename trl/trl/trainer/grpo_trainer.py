...
if is_bitsandbytes_available():
    try:
        ...
        # bitsandbytes can be "installed" but fail to import at runtime depending on the system/toolchain.
        # Treat it as unavailable in that case to keep TRL importable.
        bnb = None
else:
    bnb = None
...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                # bitsandbytes can be present but fail to import; `bnb` will be None in that case.
                if bnb is not None:
    ...
    def _post_process_rewards(
        ...
        Called inside `_generate_and_score_completions` **after** aggregated rewards are computed and **before**
        advantages are computed.

        Args:
            mode: "train" or "eval"
            inputs: original (local) dataset examples used for this batch
            prompts: prompt objects used for generation (local)
            completions: decoded completion objects used for reward functions (local)
            completion_ids_list: list-of-token-ids per completion (local)
            rewards_per_func: aggregated rewards per reward func, shape (global_batch, num_reward_funcs)
            rewards: aggregated total reward, shape (global_batch,)
        """
        # Default: no-op
        return None
    ...
    def _generate_and_score_completions(
        ...
        # Subclass hook: e.g., cross-policy success buffer updates
        self._post_process_rewards(
            mode=mode,
            inputs=inputs,
            prompts=prompts,
